...
const firstLetters = (arr) => {
  // Solution code here...
  return arr.map(string => string.slice(0, 1));
};
...
const onlyOddChars = (str) => {
  ...
  for (let i = 0; i < str.length; i++){
      i % 2 !== 0 ? tempStr = tempStr + str[i] : tempStr;
  }
  return tempStr;
};
...
const allHappy = (arr) => {
  // Solution code here...
  let happy = true;
  arr.forEach(str => str.includes(':)') ? happy : happy = false);
  return happy;
};
...
const findEvery = (arr, target) => {
  // Solution code here...
  let all = true;
  arr.forEach(str => str.includes(target) ? all : all = false);
  return all;
};
...
const unenrollBrook = (arr) => {
  ...
  let tempArr = [];
  arr.forEach(arr2 => {
      arr2.forEach(str => str.includes('Brook') ? str : tempArr.push(str));
      arr3.push(tempArr);
      tempArr = [];
  })
  return arr3;
