...
const toTitleCase = (arr) => {
  // Solution code here...
    let arr2 = arr.map(str => str[0].toUpperCase() + str.slice(1, str.length));
    return arr2;
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let arr2 =  arr.reduce((answerSoFar, person, idx) =>{
      parseInt(person.mass) > parseInt(arr[0].mass) ? answerSoFar.push(person.name) : answerSoFar;
      return answerSoFar;
  }, []);
  return arr2.join(' - ');
};
...
const sortBy = (property, arr) => {
  // Solution code here...
  property === 'name' ? arr.sort(function(a,b){
    if(a.name > b.name){
        return 1;
    }
    ...
    property === 'price' ? arr.sort(function(a,b){
        if(a.price > b.price){
            return 1;
        }
